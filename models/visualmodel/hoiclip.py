...
from .transformer import build_transformer, FeedForwardNetwork
import torchvision
...
class HOICLIP(nn.Module):
    def __init__(self, backbone, transformer, num_queries, aux_loss=False, args=None,interaction_decoder=None, ffn=None):
        super().__init__()
        ...
        self.verb2inter = MLP(args.num_verb_classes, args.clip_embed_dim // 2, args.clip_embed_dim, 3)
        ...
        if self.args.x_improved:
            self.roi_ho_downsample = nn.Linear(hidden_dim * 2, hidden_dim* 2)
            self.roi_ho_dropout = nn.Dropout(0.1)
            self.ho_feat_norm = nn.LayerNorm(hidden_dim*2)
        ...
        if self.args.interaction_decoder:
            ...
            if i_hidden_dim != hidden_dim:
                ...
                self.verb_class_embed = nn.Sequential(
                    nn.Linear(hidden_dim*2, i_hidden_dim),
            ...
            self.interaction_tgt_embed = nn.Embedding(self.args.num_verb_classes, i_hidden_dim)
            self.interaction_query_embed = nn.Embedding(self.args.num_verb_classes, i_hidden_dim)
            self.interaction_prototype_embed = nn.Identity()
            self.use_image_verb_loss =self.args.image_verb_loss
            self.interaction_decoder_layers = self.interaction_decoder.num_decoder_layers
        if self.use_image_verb_loss:
                # self.interaction_class_embed = nn.Linear(hidden_dim, 1)  # binary classification for each interaction query
                if self.args.cat_specific_fc:
                    self.interaction_class_embed = nn.Sequential(ffn, GroupWiseLinear(self.args.num_verb_classes, i_hidden_dim, bias=True))
                else:
                    self.interaction_class_embed = nn.Sequential(ffn, nn.Linear(i_hidden_dim, 1))
            # hard residual
        self.verb_class_weight = nn.Parameter(torch.Tensor(self.args.num_verb_classes, i_hidden_dim))
        self.verb_embed_norm = self.args.verb_embed_norm
    ...
    def reset_parameters(self):
        ...
        nn.init.kaiming_uniform_(self.verb_class_weight, a=math.sqrt(5))
    ...
    def forward(self, samples: NestedTensor, is_training=True, clip_input=None, targets=None):
        ...
        original_hs = original_mask.shape[1] - original_mask[:, :, 0].contiguous().sum(dim=-1)
        original_ws = original_mask.shape[2] - original_mask[:, 0, :].contiguous().sum(dim=-1)

        original_shapes = torch.stack([original_ws, original_hs], dim=-1).to(src)

        h_hs, o_hs, inter_hs, clip_cls_feature, clip_hoi_score, clip_visual,memory_ori, weight, weight_2 = self.transformer(self.input_proj(src), mask,
        ...
        if self.args.x_improved:
            feature_map = self.conv_feature_map(torch.cat([self.input_proj(src), memory_ori], dim=1)) + pos[-1]  # encoder or backbone
            n_layer, bs, _, _ = inter_hs.shape
            ...
            ho_embed = self.ho_feat_norm(inter_hs + roi_ho_embed)

        if self.args.interaction_decoder:
            i_hs,weight_3 = self.interaction_decoder(self.interaction_feature_proj(memory_ori), mask, self.interaction_tgt_embed.weight, self.interaction_query_embed.weight, self.interaction_pos_proj(pos[-1]))
            ho_embed = self.verb_class_embed(ho_embed)
            i_weight = (i_hs/32).transpose(2, 3) + self.verb_class_weight.T
            
            ho_embed = ho_embed / ho_embed.norm(dim=-1, keepdim=True)
            i_weight = i_weight / i_weight.norm(dim=-1, keepdim=True)
            logit_scale = self.logit_scale.exp()
            
            all_outputs_verb_class = logit_scale * ho_embed @ i_weight.unsqueeze(1)

            verb_class = all_outputs_verb_class[-1]
        ...
        if self.args.with_clip_label:
            ...
            verb_hs1= self.inter2verb(inter_hs)
            verb_hs2 =self.verb2inter(verb_class)
            verb_hs = verb_hs1+verb_hs2*0
            # verb_hs = verb_hs1
        ...
        root_cache = './visall'
        for hoi, boxes_h, boxes_o, tgt, attn_w, attn_wc,attn_vc in zip(out['pred_hoi_logits'], out['pred_sub_boxes'], out['pred_obj_boxes'], targets, weight, weight_2,weight_3):
            ...
            # for visualize attention map
            self.visulize_attention_ratio(image_, attn_vc[select_query], os.path.join(
                cache_dir, "{}_{}_attn_verb.png".format(tgt['filename'], score_hoi)
            ))
        ...
        if self.aux_loss:
            ...
            out['aux_outputs'] = self._set_aux_loss_triplet(outputs_hoi_class, outputs_obj_class,
                                                            outputs_sub_coord, outputs_obj_coord,verb_class,
                                                            aux_mimic)
    ...
    def _set_aux_loss_triplet(self, outputs_hoi_class, outputs_obj_class,
                              outputs_sub_coord, outputs_obj_coord, verb_class,outputs_inter_hs=None):
        ...
        aux_outputs = {'pred_hoi_logits': outputs_hoi_class[-self.dec_layers: -1],
                       'pred_obj_logits': outputs_obj_class[-self.dec_layers: -1],
                        'pred_verb_logits':verb_class[-self.dec_layers: -1],
