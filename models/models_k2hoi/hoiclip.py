...
class HOICLIP(nn.Module):
    ...
    def forward(self, samples: NestedTensor, is_training=True, clip_input=None, targets=None):
        ...
        h_hs, o_hs, inter_hs, clip_cls_feature, clip_hoi_score, clip_visual,memory_ori = self.transformer(self.input_proj(src), mask,
        ...
        if self.args.interaction_decoder:
            i_hs = self.interaction_decoder(self.interaction_feature_proj(memory_ori), mask, self.interaction_tgt_embed.weight, self.interaction_query_embed.weight, self.interaction_pos_proj(pos[-1]))
...
class SetCriterionHOI(nn.Module):
    ...
    def reconstruction_loss(self, outputs, targets, indices, num_interactions):
        ...
        loss_rec1 = F.mse_loss(raw_feature, hoi_feature)
        loss_rec2= F.mse_loss(raw_feature, hoi_feature)
        loss_rec=loss_rec1*0.5+loss_rec2*0.5
