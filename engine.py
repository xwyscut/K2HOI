...
def evaluate_hoi(dataset_file, model, postprocessors, data_loader,
    ...
    if args.training_free_enhancement_path:
        ...
        text_hoi_feature = model.module.transformer.hoi_cls  #加上module
     
        spatial_feature = torch.cat([i['clip_visual'].unsqueeze(0) for i in preds]).to(device)
        spatial_feature /= spatial_feature.norm(dim=-1, keepdim=True)
        spatial_cls = spatial_feature[:, 0, :].contiguous()  # M, c

        cls_scores = (spatial_cls @ text_hoi_feature.T).to(device)
        with open(os.path.join(root, 'log.txt'), 'a') as f:
        ...
        for a in [1]:
            for co in [0.5]:
                for topk in [10]:
                    ...
                    test_pred = copy.deepcopy(preds)
                    clip_hoi_score = cls_scores.to(device)
                    ...
                    for i in range(len(test_pred)):
                        test_pred[i]['hoi_scores'] = test_pred[i]['hoi_scores'].to(device)
